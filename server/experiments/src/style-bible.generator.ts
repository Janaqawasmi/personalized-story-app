/**
 * Claude calls for the Style Bible framework (exp-04).
 *
 * Two calls:
 *   callClaudeForStyleBible  — reads ALL story pages, produces the full StyleBible
 *   callClaudeForStructuredScenePrompts — reads TARGET pages + locked StyleBible,
 *                              produces 5-section structured scene prompts
 */
...
export async function callClaudeForStyleBible(
  ...
  const required = ["characterSheet", "characterAnchor", "styleGuide", "consistencyAnchors", "environmentRegistry", "palette", "avoidList"];
  for (const key of required) {
    if (!(key in parsed)) throw new Error(`callClaudeForStyleBible: missing field "${key}"`);
  }
...
// ---------------------------------------------------------------------------
// Structured scene prompt generation
// ---------------------------------------------------------------------------

function buildStructuredScenePromptsPrompt(
  pages: PageIllustration[],
  bible: StyleBible,
): string {
  ...
  return `You are a children's book art director. The Style Bible is locked. Generate one structured SCENE PROMPT per page.
...
Each scene prompt has exactly 5 fields. Be CONCISE — every field must be under 20 words.

setting (under 20 words):
  "<registry key> | <light state> | <2 props from spatialLayout with positions>"
  Use the EXACT registry key. Only name props from spatialLayout.

character (under 20 words):
  "<furniture they're on/in> | <body language: 2–3 physical specifics>"
  NEVER name emotions. Only physical specifics: "arms rigid", "palms pressing mattress", "gaze fixed on corner".
  Always explicitly place the character on a named piece of furniture.
...
lighting (under 25 words):
  "<source + position> | <quality> | <what's lit> | <what's in shadow> | mood: <word>"
  Mandatory. In dark scenes, describe near-absence of light with the same precision as bright light.

CRITICAL RULES:
- NEVER carry metaphors from the story text into the prompt. Translate them to literal visuals. "A small dark mountain" of books → "a loose pile of closed books stacked unevenly on the floor".
- In dark scenes, the avoid list item about mountain-shaped silhouettes applies: books are books, not a mountain shape.
- The character must always be explicitly on/in a named piece of furniture.
...
export async function callClaudeForStructuredScenePrompts(
  ...
  const textBlock = response.content.find((b) => b.type === "text");
  if (!textBlock || textBlock.type !== "text") {
    throw new Error("callClaudeForStructuredScenePrompts: Claude returned no text block");
  }
  ...
  try {
    ...
    throw new Error(
      `callClaudeForStructuredScenePrompts: failed to parse JSON. Raw: ${raw.slice(0, 300)}`,
    );
  }

  if (!Array.isArray(parsed.scenePrompts) || parsed.scenePrompts.length !== pages.length) {
    throw new Error(
      `callClaudeForStructuredScenePrompts: expected ${pages.length} prompts, got ${parsed.scenePrompts?.length}`,
    );
  }
