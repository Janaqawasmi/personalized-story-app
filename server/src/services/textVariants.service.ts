...
export async function generateTextVariants(templateId: string): Promise<void> {
  ...
  // Validate every variant before writing anything — a page that drops a
  // required placeholder must abort the whole batch rather than publish
  // half-good personalization data.
  try {
    for (const variant of variants) {
      const original =
        pageInputs.find((p) => p.pageNumber === variant.pageNumber)?.text ?? "";
      assertVariantValid(original, variant.masculine, `Page ${variant.pageNumber} masculine`);
      assertVariantValid(original, variant.feminine, `Page ${variant.pageNumber} feminine`);
    }
  } catch (err) {
    await templateRef.update({ textVariantStatus: "none" });
    throw err;
  ...
  const updatedPages = pages.map((page) => {
    ...
    const variant = variants.find((v) => v.pageNumber === pn);
