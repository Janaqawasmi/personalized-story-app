...
export function buildStoryDraftPrompt(
  ...
  sections.push("Your output must be therapeutically sound and emotionally gentle, safe, and therapeutically sound.");
  ...
  sections.push("   {");
  sections.push('     "title": "string (Arabic only, no English words)",');
  sections.push('     "pages": [');
  sections.push("       {");
  sections.push('         "pageNumber": number (1-based, sequential),');
  sections.push('         "text": "string (Arabic only, use {{child_name}} placeholder)",');
  sections.push('         "emotionalTone": "string (must be one of: gentle | reassuring | calm)",');
