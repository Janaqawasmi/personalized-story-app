/**
 * Two-call creative director pipeline — production copy for the pilot service.
 *
 * Mirrors server/experiments/src/scene-director.ts. Pilot owns its own copy so
 * the experiments folder can evolve independently and is excluded from the
 * production build. If you change one, consider porting the change to the
 * other.
 *
 * Call 1 — SCENE DIRECTOR (creative)
 *   Input:  page text + story brief + style bible + mode
 *   Output: SceneDirection — moment, emotion_for_viewer, key_physical_detail,
 *           visual_hook, camera
 *
 * Call 2 — PROMPT CONVERTER (technical)
 *   Input:  SceneDirection + style bible + page text + mode
 *   Output: ScenePromptSections (5-section structured format)
 */
