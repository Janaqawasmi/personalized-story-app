...
export async function regenerateScenePlan(params: {
  storyId: string;
  pageNumber: number;
  uid: string;
  ...
  if (story.status !== "illustration_workspace") {
    throw new IllegalStateGenerateImageError(
      `regenerateScenePlan requires illustration_workspace (got ${story.status})`,
    );
  }
  ...
  return { scenePlanId: newPlan.id, version: newPlan.version };
}
