...
Complexity is handled through four layers — a live meter for continuous awareness, a collaborative length-bump suggestion for the easiest fix, a pre-submit warning as final check, and post-draft metadata for transparency. Each layer triggers at a distinct moment so the psychologist never sees the same overload message twice. No hard caps, no silent auto-bumps, no presets. The psychologist stays in control while the system makes the problem visible at the right moments.
...
The complexity handling strategy has **four layers**. Each layer triggers at a distinct moment so the psychologist never sees the same overload message twice in one session.

**Layer 1 — Live Complexity Meter.** A persistent visual progress bar visible across all form sections. **Label:** "Story load" (not page numbers, not percentages). The bar fills as clinical elements are added, using the weighted page costs and age-range multipliers from Section 16. The bar fills against **budget.max** as the denominator, with a visible tick mark at **budget.min**. The fill ratio alone does not indicate overload — **color state is the signal.** **Three color states**, defined against the **same page budget band** as Section 16 for the selected age range × story length (the **minimum** and **maximum** page counts in that section's budget table — not as fixed fractions of the maximum alone): **green** when the total estimated page cost is **at or below the minimum** of that range; **yellow** when the total is **above the minimum but at or below the maximum** (the Section 16 soft-overload situation: past the lower bound of the chosen length, still within its published upper bound); **red** when the total is **above the maximum**. The default view shows only the bar and the label — no math, no page numbers. When clicked or hovered, the bar expands to show the underlying breakdown: each obligation contributing to the load, total page cost, and available budget for the selected age range × story length. The expanded view is the only place where page numbers appear.
...
**Layer 3 — Pre-Submit Warning.** A soft warning shown at submission time — but **only if** load is in **red** state (total estimated page cost **above the maximum** of the page range for the selected age × length; see Layer 1), the psychologist has **not** already acknowledged the overload through the length bump suggestion, **and** story length is **not already Extended**. When the story length is already **Extended** and load is still **red**, the pre-submit warning is suppressed — the system has no further length action to offer and the psychologist has already been informed through the meter. If no earlier signal fired (edge case), show the warning with the obligation breakdown. The psychologist can acknowledge and proceed. The substantive warning behavior and copy remain as defined in Section 16; this subsection adds the **de-duplication rule** so earlier acknowledgments suppress the submit-time repeat.

**Layer 4 — Post-Draft Transparency Metadata.** When the agent generates a draft, it outputs metadata listing what was fully included, what was compressed, and what was omitted. This is the safety net for edge cases. Specified in Section 15 (output metadata requirement); unchanged here. **Implementation note:** This layer is not yet implemented in V1 and is out of scope until the generation and review pipeline exists (see `docs/story-brief-implementation-gaps.md`).
...
- The pre-submit warning fires only if load is red, story length is not already Extended, **and** the length bump has not been acknowledged.
